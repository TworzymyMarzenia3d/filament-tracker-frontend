...
function MainApp({ token, onLogout }) {
  ...
  // --- Stany dla formularza zużycia ---
  const [jobName, setJobName] = useState('');
  const [usageEntries, setUsageEntries] = useState([
    { filamentTypeId: '', weightToUse: '' }
  ]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (typesData.length > 0 && usageEntries[0].filamentTypeId === '') {
        const updatedEntries = [...usageEntries];
        updatedEntries[0].filamentTypeId = typesData[0].id;
  ...
  // --- Funkcje do obsługi dynamicznego formularza zużycia ---
  ...
  const addUsageEntry = () => {
    const firstFilamentId = filamentTypes.length > 0 ? filamentTypes[0].id : '';
    setUsageEntries([...usageEntries, { filamentTypeId: firstFilamentId, weightToUse: '' }]);
  };

  const removeUsageEntry = (index) => {
    const newEntries = usageEntries.filter((_, i) => i !== index);
    setUsageEntries(newEntries);
  };

  const handleLogUsage = async (e) => {
    e.preventDefault();
    const validEntries = usageEntries.filter(entry => entry.filamentTypeId && entry.weightToUse > 0);
    if (!jobName || validEntries.length === 0) {
        alert("Proszę podać nazwę zlecenia i wypełnić co najmniej jedną pozycję zużycia.");
        return;
    }
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Nie udało się zapisać zużycia.');
        }
        alert('Zużycie zostało pomyślnie zapisane!');
        fetchData();
        setJobName('');
        setUsageEntries([{ filamentTypeId: filamentTypes.length > 0 ? filamentTypes[0].id : '', weightToUse: '' }]);
    } catch (err) {
    ...
    <div className="App">
      ...
      <main className="container">
        ...
        <section className="form-section full-width">
            <h2>Zarejestruj zużycie materiału (Wydruk)</h2>
            <form onSubmit={handleLogUsage}>
                <input type="text" placeholder="Nazwa zlecenia / Klient" value={jobName} onChange={(e) => setJobName(e.target.value)} required className="job-name-input" />
                {usageEntries.map((entry, index) => (
                    <div key={index} className="usage-entry">
                        <select value={entry.filamentTypeId} onChange={(e) => handleUsageChange(index, 'filamentTypeId', e.target.value)} required>
                            <option value="" disabled>-- Wybierz filament --</option>
                            {filamentTypes.map(ft => <option key={ft.id} value={ft.id}>{ft.manufacturer} {ft.material} {ft.color}</option>)}
                        </select>
                        <input type="number" placeholder="Zużyta waga (g)" value={entry.weightToUse} onChange={(e) => handleUsageChange(index, 'weightToUse', e.target.value)} required />
                        {usageEntries.length > 1 && <button type="button" onClick={() => removeUsageEntry(index)} className="remove-btn">Usuń</button>}
                    </div>
                ))}
                <button type="button" onClick={addUsageEntry} className="add-btn">+ Dodaj kolejną pozycję</button>
                <button type="submit" className="submit-job-btn">Zapisz całe zlecenie</button>
            </form>
        </section>
        ...
        <section className="list-section full-width">
          ...
          {isLoading ? <p>Ładowanie...</p> : error ? <p className="error-message">{error}</p> : (
            <table>
              <thead>
                <tr>
                  <th>Filament</th><th>Data zakupu</th><th>Waga (zostało / całość)</th><th>Cena zakupu (oryg.)</th><th>Koszt / gram (PLN)</th>
                </tr>
              </thead>
              <tbody>
